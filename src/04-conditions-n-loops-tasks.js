...
function isCreditCardNumber(ccn) {
  let sum = 0;
  let n = 0;
  let str = ccn.toString().replace(/\D/g, '');
  if (str.length % 2 === 0) str = `0${str}`;

  for (let i = 0; i < str.length; i += 1) {
    if (i % 2 !== 0) {
      if (parseInt(str[i], 10) * 2 > 9) {
        n = parseInt(str[i], 10) * 2 - 9;
      } else n = parseInt(str[i], 10) * 2;
    } else n = parseInt(str[i], 10);
...
function getDigitalRoot(num) {
  function sum(number) {
    ...
    const result = arr.reduce((sumCurrent, current) => sumCurrent + current, 0);

    if (result > 9) return sum(result);
...
function getCommonDirectoryPath(pathes) {
  const result = [];
  const arrayPathes = pathes.map((item) => item.split('/'));
  for (let i = 0; i < arrayPathes[0].length; i += 1) {
    const partArray = arrayPathes[0][i];
    if (arrayPathes.every((current) => current[i] === partArray)) {
